...
MoTP <- function(data_list, omics_list = c("mRNA", "miRNA", "lncRNA", "DNA-methylation")) {
  ...
  process_data <- function(data, Fit, neurons) {
    training_data <- Fit[[1]]$trainingData
    features <- intersect(colnames(training_data)[-1], colnames(data))
    
    #Contain at least 20% of the required features for the model 
    if (length(features) < length(colnames(training_data)[-1])*0.2) {
    ...
    } else if (length(features) < length(colnames(training_data)[-1])) {
